...
options = {
  ...
  map_frame = "map",
  tracking_frame = "imu_link", --imu_frame
  published_frame = "base_footprint",
  odom_frame = "odom",
  provide_odom_frame = true,
  publish_frame_projected_to_2d = true, -- false
  publish_tracked_pose = true,
  use_odometry =true,  -- 211111
  use_pose_extrapolator = false, -- false
  use_nav_sat = false,
  use_landmarks = false,
  num_laser_scans = 2,
  num_multi_echo_laser_scans = 0,
  num_subdivisions_per_laser_scan = 8, -- 이 값 변경하면 odometry 오류 해결 가능
  num_point_clouds = 0,
...
TRAJECTORY_BUILDER_2D.num_accumulated_range_data = 16 -- 2 / num laser  scnas * num subdivisions per laser scan
TRAJECTORY_BUILDER_2D.use_imu_data = true
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 45 -- 45 

-- 211111
--TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = 10
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 60 -- 수정
-- IMU is ok
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 60 -- 수정
